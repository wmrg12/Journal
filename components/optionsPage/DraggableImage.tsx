...
export const DraggableImage: React.FC<Props> = ({
  ...
  // sync Animated pan when props change
  useEffect(() => {
    Animated.timing(pan, {
  ...
  useEffect(() => {
    if (!hasValidatedPosition.current && canvasWidth > 0 && canvasHeight > 0 && size.width > 0 && size.height > 0) {
      ...
      if (validX !== currentX || validY !== currentY) {
        ...
        // Persistimos el ajuste inicial en la BD
        (async () => {
          try {
  ...
  const panResponder = useRef(
    PanResponder.create({
      ...
      onPanResponderRelease: async (_evt, g) => {
        ...
        // Persistir en BD (solo al soltar)
        try {
  ...
  /* ---------- Resize panResponder (guarda en BD al soltar) ---------- */
